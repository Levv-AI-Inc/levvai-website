...
function hasPositiveNumber(value: string | number | undefined): boolean {
...
export default function CommercialsPage() {
  ...
  // Fixed fee
  const [fixedAmount,      setFixedAmount]      = useState<string>(toInputValue(c.fixedAmount))
  ...
  // Recurring
  const [recurringAmount,  setRecurringAmount]  = useState<string>(toInputValue(c.recurringAmount))
  ...
  // Cost-plus
  const [costBase,         setCostBase]         = useState<string>(toInputValue(c.costBase))
  const [markupPct,        setMarkupPct]        = useState<string>(toInputValue(c.markupPct))
  ...
  const canContinue = (() => {
    switch (pricingModel) {
      case 'Fixed fee':
        return hasPositiveNumber(fixedAmount)
      case 'Milestone-based':
        return milestones.length > 0 && milestones.every((milestone) =>
          milestone.name.trim().length > 0 &&
          milestone.due.trim().length > 0 &&
          milestone.amount > 0
        )
      case 'Time & materials':
        return tmRoles.length > 0 && tmRoles.every((role) =>
          role.role.trim().length > 0 &&
          role.rate > 0 &&
          role.startDate.trim().length > 0 &&
          role.endDate.trim().length > 0
        )
      case 'Recurring':
        return hasPositiveNumber(recurringAmount) && billingFrequency.trim().length > 0
      case 'Hybrid':
        return (
          hasPositiveNumber(fixedAmount) ||
          (tmRoles.length > 0 && tmRoles.every((role) =>
            role.role.trim().length > 0 &&
            role.rate > 0 &&
            role.startDate.trim().length > 0 &&
            role.endDate.trim().length > 0
      ...
      case 'Cost-plus':
        return hasPositiveNumber(costBase) && hasPositiveNumber(markupPct)
      default:
        return false
    }
  })()
