    ...
    <div className="p-8 bg-slate-50 min-h-screen text-slate-900 font-sans relative overflow-hidden">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">SOWs</h1>
            <p className="text-slate-500 font-medium mt-1">Manage Statements of Work and engagement performance.</p>
          </div>
        ...
        {/* TOP SECTION: Financials + Advanced Filters */}
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col justify-center">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Filtered Value</span>
                  ...
                  filteredSOWs.map((sow) => (
                    ...
                    >
                      <td className="px-8 py-6">
                        <div className="text-[10px] font-black text-cyan-600 bg-cyan-50 w-fit px-2 py-0.5 rounded-md mb-1.5 uppercase leading-none">{sow.id}</div>
                        <div className="font-bold text-slate-900 leading-tight">{sow.title}</div>
                      </td>
                      <td className="px-8 py-6">
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8">
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                  <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Preview Value</p>
                ...
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                  <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Engagement Status</p>
                  <p className="text-sm font-bold text-slate-900 uppercase mt-1">{selectedRecord.status}</p>
                </div>
              </div>

              <section>
                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-[0.2em] mb-3">Engagement Brief</h3>
                <p className="text-slate-600 leading-relaxed font-medium">{selectedRecord.description}</p>
              </section>

              <section className="grid grid-cols-2 gap-6">
                <div>
                  <h3 className="text-xs font-bold text-slate-400 uppercase tracking-[0.2em] mb-3 flex items-center gap-2"><User size={14} /> Contact</h3>
                  <p className="text-sm font-bold text-slate-900">{selectedRecord.supplier}</p>
                  <p className="text-xs text-slate-500">{selectedRecord.manager}</p>
                </div>
                <div>
              ...
              <div className="pt-6 border-t border-slate-100">
                <button className="w-full bg-slate-950 text-white py-4 rounded-2xl font-bold flex items-center justify-center gap-3 hover:bg-black transition-all group shadow-xl shadow-slate-200">
                   View Full SOW Record <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
                </button>
                <button className="w-full mt-3 bg-white border border-slate-200 text-slate-600 py-3 rounded-2xl font-bold text-sm hover:bg-slate-50 transition-all flex items-center justify-center gap-2">
                  <ExternalLink size={14} /> Open SOW Document
                </button>
              </div>
            </div>
          </div>
